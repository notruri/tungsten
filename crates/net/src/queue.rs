...
use self::files::{
    apply_inferred_destination_file_name, remove_file_if_exists, resolve_destination, temp_path_for,
};
...
pub(super) struct RuntimeDownloadState {
    pub(super) progress: ProgressSnapshot,
...
impl RuntimeDownloadState {
    pub(super) fn new(progress: ProgressSnapshot) -> Self {
        Self {
            progress,
            ui_dirty: false,
            persist_dirty: false,

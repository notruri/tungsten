...
async fn main() -> Result<()> {
    ...
    let _daemon = Daemon::new(Arc::new(runtime));
...
impl DaemonSettings {
    ...
    fn normalize(mut self) -> Self {
        if self.temp_dir.as_os_str().is_empty() {
            self.temp_dir = std::env::temp_dir().join(APP_DIR);
        }
...
impl SettingsFile {
    fn into_settings(self, defaults: &DaemonSettings) -> DaemonSettings {
        DaemonSettings {
...
fn load_settings() -> Result<DaemonSettings> {
    let defaults = DaemonSettings::defaults()?;
